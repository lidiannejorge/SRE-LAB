...
# ==========================
resource = Resource.create({"service.name": os.getenv("OTEL_SERVICE_NAME", "pedido-service")})
trace.set_tracer_provider(TracerProvider(resource=resource))
tracer = trace.get_tracer("pedido-service")
...
def health():
    with tracer.start_as_current_span("pedido-health"):
        logger.info("Health check pedido-service")
        return jsonify({"status": "UP", "service": "pedido-service"})
...
def listar_pedidos():
    with tracer.start_as_current_span("listar-pedidos"):
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, cliente_id, produto, valor, criado_em FROM pedidos")
        pedidos = [
            {"id": row[0], "cliente_id": row[1], "produto": row[2], "valor": float(row[3]), "criado_em": str(row[4])}
            for row in cursor.fetchall()
        ]
        cursor.close()
        conn.close()
        REQUEST_COUNT.labels("GET", "/pedidos", "200").inc()
...
def criar_pedido():
    with tracer.start_as_current_span("criar-pedido"):
        ...
        cursor.execute(
            ...
            RETURNING id
            """,
            (dados["cliente_id"], dados["produto"], dados["valor"])
        )
        pedido_id = cursor.fetchone()[0]
        conn.commit()
        cursor.close()
        conn.close()
        REQUEST_COUNT.labels("POST", "/pedidos", "201").inc()
        logger.info(f"Pedido criado com id={pedido_id}")
        return jsonify({"id": pedido_id, "status": "created"}), 201
